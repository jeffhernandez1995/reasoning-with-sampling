...
if __name__ == "__main__":
    ...
    parser.add_argument("--resample_interval", type=int, default=8)
    parser.add_argument(
        "--resample_method",
        type=str,
        default="systematic",
    ...
    smc_method_config: Dict[str, Any] = {
    ...
    print(
        (
            f"Starting MATH(power_smc): batch_idx={args.batch_idx} seed={args.seed} "
    ...
    for step_idx, data in enumerate(tqdm(dataset[start:end], desc=f"Benchmark on MATH ({args.sampling_method})")):
        ...
        try:
            ...
            method_sample = sampler.sample_method(
                ...
                mcmc_steps=0,
                max_new_tokens=args.max_new_tokens,
                method_config=smc_method_config,
            )
            if args.cuda_sync and torch.cuda.is_available():
                torch.cuda.synchronize()
            if args.debug_verbose:
                print(
                    (
                        f"[step {step_idx + 1}] power_smc latency={method_sample.latency_seconds:.2f}s "
            ...
            results.append(
                {
                    ...
                    "smc_generated_completion": method_sample.completion,
                    "smc_answer": method_answer,
            ...
            wandb_logger.log_sample(
                {
                    ...
                    "smc_completion": truncate_text(method_sample.completion),
                    "naive_reward": naive_reward,
                    "std_reward": std_reward,
                    "smc_reward": method_reward,
                    "base_sampling_seconds": base_seconds,
    ...
    print(f"Average power_smc sampling time per sample: {avg_smc_seconds:.4f} seconds")
    ...
    wandb_logger.finish(
        summary={
            ...
            "summary/avg_power_sampling_seconds": avg_smc_seconds,
        }
    )
